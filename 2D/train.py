...
if __name__ == "__main__":
    ...
    input_img = torch.rand((1,3,224,224)).cuda()
    flops = FlopCountAnalysis(net, input_img)
    print("Total Flops: {}G".format(flops.total()*1e-9))
    param_train =  sum(p.numel() for p in net.parameters() if p.requires_grad)
    print("Trainable params: {}M".format(param_train * 1e-6))
    import sys
    sys.exit()
