...
from networks.VAN_FC import  MaxViT_deformableLKAFormer
...
parser.add_argument("--visualize", action="store_true", help="Visualize input and output images")
...
def visualize_input_output(image, label, output):
    # Convert tensors to numpy arrays
    image = image.squeeze().cpu().numpy()
    label = label.squeeze().cpu().numpy()
    output = output.argmax(dim=1).squeeze().cpu().numpy()

    # Plot input, label, and output images
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        if args.visualize:
            visualize_input_output(image, label, output)
