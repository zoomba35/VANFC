...
class MaxViT4Out_Small(nn.Module):
    def __init__(self, n_class=1, img_size=224, pretrain=True):
        ...
        if img_size==224:
            ...
            if pretrain:
                print('Loading:', './pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
                # state_dict = torch.load('./pretrained_pth/maxvit/maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth')
                state_dict = torch.load(r"F:\codes\deformableLKA-main\2D\pretrained_pth\maxvit\maxvit_rmlp_small_rw_224_sw-6ef0ae4f.pth")
    ...
    def forward(self, x):
        ...
        # if grayscale input, convert to 3 channels
        # print("xsds",x.shape)  # torch.Size([1, 3, 224, 224])
        # print(x.size()[1])  # tensor(3)
        if x.size()[1] == 1:
            x = self.conv(x)
            #print("xsds1",x.shape)  # 因为x.size()[1]=3，所以没有进入这一步
    ##########f[3] 是第四个特征图，f[2] 是第三个特征图，f[1] 是第二个特征图，f[0] 是第一个特征图###########
        # transformer backbone as encoder
        f = self.backbone(x)
        #print("xsds2",x.shape)  # xsds2 torch.Size([1, 3, 224, 224])

        x1_o, x2_o, x3_o, x4_o = f[3], f[2], f[1], f[0]
        #print([x1_o.shape,x2_o.shape,x3_o.shape,x4_o.shape])
        # [torch.Size([1, 768, 7, 7]), torch.Size([1, 384, 14, 14]), torch.Size([1, 192, 28, 28]), torch.Size([1, 96, 56, 56])]
    #######这四个特征图分别被赋值给变量 x1_o、x2_o、x3_o、x4_o#######
